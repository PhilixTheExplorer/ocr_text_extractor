...
- Splitting a spread PDF whose pages carry a rotation flag no longer drops the
  far half as a blank page, and re-splitting an already-split PDF no longer
  produces duplicate halves or loses content. Each half is now taken from the
  displayed page and anchored to the current cropbox.
